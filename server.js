                ...
                const { rows } = await pool.query(
                    `UPDATE companies 
                     SET market_intelligence = $1, 
                         ...
                         fit_score = CASE 
                            WHEN $1 ILIKE '%High fit%' OR $1 ILIKE '%Excellent fit%' THEN 9 
                            WHEN $1 ILIKE '%Medium fit%' OR $1 ILIKE '%Good fit%' THEN 7 
                            WHEN $1 ILIKE '%Low fit%' OR $1 ILIKE '%Poor fit%' THEN 4
                            ELSE fit_score 
                         END
...
// --- APIFY INTEGRATION ---
import { startLeadsScraper, checkApifyRun, getApifyResults } from './src/backend/services/apify.js';
...
app.post('/api/integrations/apify/run', async (req, res) => {
    ...
    try {
        const runId = await startLeadsScraper(effectiveToken, domains, filters);
        res.json({ runId });
    } catch (error) {
