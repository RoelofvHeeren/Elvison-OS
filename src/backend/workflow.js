...
export const runAgentWorkflow = async (input, config) => {
    ...
    const checkCancellation = async () => {
        const runId = idempotencyKey; // Use idempotencyKey as runId for consistency
        const currentLogs = []; // Initialize logs array

        const logStep = (step, message) => {
            if (listeners?.onLog) listeners.onLog({ step, detail: message });
            else console.log(`[${step}] ${message}`);

            if (step !== 'System') {
                currentLogs.push({ step, message, timestamp: new Date().toISOString() });
                // Incremental save could go here if needed, but for now we rely on the final try/catch block
            }
        };
...
STRICTURE: LLM is fallback only. Zero creativity. If data is unsalvageable, mark is_valid: false.`,
            ...
            try {
                ...
                // Fallback handled by try/catch logic if needed
