...
function writeEnvJsonToFile(envVar, targetPath) {
    const raw = process.env[envVar];
    if (!raw) {
        return false;
    }
    const decoded = decodeJsonEnv(raw);
...
async function authenticateAndSaveCredentials() {
    ...
    // Allow shipping credentials via env to avoid baking secrets into the image.
    writeEnvJsonToFile("GSHEETS_OAUTH_JSON", gcpKeysPath);
    if (!fs.existsSync(gcpKeysPath)) {
        console.error("GCP keys not found. Please create your credentials in Google Cloud then copy `gcp-oauth.keys.json` into your ./dist directory.");
        process.exit(1);
    }
...
async function startSseServer() {
    ...
    const httpServer = http.createServer(async (req, res) => {
        ...
        if (req.method === "GET" && url.pathname === "/sse") {
            ...
            try {
                ...
                console.error("Failed to establish SSE transport", error?.message || error);
                if (!res.headersSent) {
                    res.writeHead(500).end("Failed to start SSE transport");
                }
                sessions.delete(transport.sessionId);
            }
            return;
    ...
    await new Promise((resolve) => {
        httpServer.listen(port, host, () => {
            ...
            console.error("Connect your client to /sse; POSTs will go to /messages (announced via SSE endpoint event).");
            resolve();
        });
    });
}
async function loadCredentialsAndRunServer() {
    let auth;
    // If credentials are provided via env, prefer them (this avoids the interactive flow on hosted environments).
    const envCredentials = process.env.GSHEETS_CREDENTIALS_JSON;
    if (envCredentials) {
        try {
            const decoded = decodeJsonEnv(envCredentials);
            const parsed = JSON.parse(decoded);
            auth = new google.auth.OAuth2();
            auth.setCredentials(parsed);
            google.options({ auth });
            console.error("Loaded Google credentials from GSHEETS_CREDENTIALS_JSON.");
            await startServerWithConfiguredTransport();
            return;
        }
        catch (error) {
            console.error("Failed to load GSHEETS_CREDENTIALS_JSON, falling back to file/auth flow.", error?.message || error);
        }
    }
...
async function startServerWithConfiguredTransport() {
